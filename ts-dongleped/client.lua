...
RegisterNetEvent('ts-dongle:activity', function()
            local header = {
                {
                    isMenuHeader = true,
            ...
            exports['ts-menu']:openMenu(header)
        end)

RegisterNetEvent('ts-dongle:buyitems', function(data)
    local header = {
        {
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
    ...
    for k, v in pairs(Config.Shop) do
        if TSCore.Shared.Items[v.item].label then
            header[#header+1] = {
                header = TSCore.Shared.Items[v.item].label,
                txt = "Price: "..v.price,
                icon = v.icon,
                params = {
                    isServer = true,
                    event = "ts-dongle:server:buyshit",
                    args = k
                }
            }
        end
...
-- do something like when a TriggerClientEvent('ts-dongle:client:SetBankCD', -1, true) when a bank is being robbed
-- do something like TriggerClientEvent('ts-dongle:client:SetBankCD', -1, false) when a bank can be hit again

RegisterNetEvent('ts-dongle:client:SetBankCD', function(bool)
    BankRobberyCD = bool
end)
...
CreateThread(function()
    exports['ts-target']:AddTargetModel('ig_djgeneric_01', {
        options = {
            { 
                type = "client",
                event = "ts-dongle:activity",
                icon = "fas fa-circle",
